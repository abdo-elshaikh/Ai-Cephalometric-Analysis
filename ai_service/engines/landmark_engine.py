...
class ScientificRefiner:
    ...
    def refine(
            ...
            if dx != 0:
                ...
                if abs(fh_angle) > 25:
                    ...
                    for lm in landmarks.values():
                        lm.confidence = round(lm.confidence * 0.90, 4)

                # Validate Frankfort plane length when scale is known
                if has_scale:
        ...
        if has_scale and "S" in landmarks and "N" in landmarks:
            ...
            if lo <= sn_mm <= hi:
                # Valid SN length → boost confidence of both points
                for k in ("S", "N"):
        ...
        # ── 6. Derive Menton (Me) from Gnathion & Gonion ──────────────────────
